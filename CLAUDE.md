...
**Font.** `src/assets/fonts/peridot-pe-variable.woff2` (40 KB) is generated by `tools/build-font.py` from the foundry OTF in `tools/fonts/` (not shipped). Rebuild it there if the site ever needs another script or weight; never point `fonts.css` at the OTF.
