...
**URLs and crawlers.** Every asset URL is root-absolute (`deployUrl: "/"` in `angular.json`, `/assets/…` in configs and templates, `/favicon.ico`). `<base href="/">` resolves relative ones for browsers and Googlebot, but GPTBot requested `/blog/chunk-….js` and `/blog/category/assets/…` from the blog pages — each a `blog/:slug` render and an API call. `server.ts` now answers any leftover file-like path with a bare 404 before Angular sees it. The auth and signed-in zones answer with `X-Robots-Tag: noindex` from `server.ts`; `/auth` is deliberately not in `robots.txt` (every page links it, and a blocked-but-linked URL gets indexed as a bare address), `/user` and `/admin` are.

**The blog API and the server.** Every blog page is rendered by asking the API from the droplet's single address, so `server.ts` wraps the global `fetch` (HttpClient runs on `withFetch()`) and keeps `api/blog/*` answers for a minute, serving a copy up to a day old when the API answers 429/5xx — a crawl burst used to turn the whole blog into a 503. The API side gives `api/blog/*` its own limiter (300/min per address) and `Cache-Control: public, max-age=60`. `server.ts` also generates `/sitemap.xml`, `/rss.xml` and `/llms.txt` (description plus every article) from the API, cached an hour. Archives with fewer than three posts are `noindex, follow` and left out of the sitemap (`INDEXABLE_CATEGORY_MIN_POSTS`, kept in step in `blog.mapper.ts` and `server.ts`). Cloudflare (Free) sits in front of `beehivemind.tech` since 2026-09-11 and caches HTML for the five minutes the app declares; `api.` is DNS-only so Laravel keeps seeing real addresses.
...
**Google Tag Manager is the only measurement script this app loads** (`environment.googleTagManagerId`, prod only); GA4 lives inside the container, so a tag or a parameter changes without a deploy. One rule: **application code never calls `gtag` directly** — it pushes to the `dataLayer` through `AnalyticsService`, in one shape:

```js
dataLayer.push({ ga4: null });                                   // the previous event's parameters, cleared
dataLayer.push({ event: 'ga4', ga4_event: 'sign_up', ga4: { method: 'email' } });
```

The reset is not optional: GTM merges pushes, so without it `method` would ride along with the next event. Context that *should* persist (`page_type`, `content_group`, `user_id`, `user_country`, `user_auth_method`, `traffic_type`) is pushed on its own, outside `ga4`.
...
- **Audit:** the handoff's `audit/checks.js` battery (overflow, weights, contrast, off-palette, disabled opacity, hit targets, clipping, measure, dialog contract) is run against every route at 375/768/1024/1440 and must report zero findings. The harness is in `audit/`: `cd audit && npm install`, start `ng serve --port 4301`, then `npm run audit` (all routes; `-- --only=/user/beehives,admin` narrows, `--dialogs=0` skips the dialog contract) or `npm run probe -- <path> <width> [user|admin]` to list what overflows on one page. It drives the system Chrome through puppeteer-core, mocks `localhost:8000/api/*` from `mocks.mjs`, and seeds `localStorage.bhm_auth` for the user/admin zones. Known blind spot: `color-mix()` grounds (calendar chips) come back as `color(srgb …)`, which the battery cannot parse, so they are not measured. A second blind spot to be aware of: the battery finds nothing on a page that never rendered, so a **404 passes** — `npm run routes` (`audit/routes-check.mjs`) asserts that every zone lands on its own layout before the visual run means anything. `npm run editor` (`audit/editor-smoke.mjs`) drives the blog editor for the parts a static check cannot see: TipTap booting, the toolbar acting on a selection, and the OG card being drawn. `npm run selects` (`audit/select-check.mjs`) opens Add Inspection / Feeding / Harvest and asserts their selects list options and the beehive select follows the apiary — both broke silently once (options set outside change detection in this zoneless app; a bound `[multiple]` turning every select into a multi-select that returned `[1]`), and no visual check notices an empty dropdown. `npm run invite-logout` (`audit/invite-logout-check.mjs`, dev server on 4321 — 4301 can collide with other local projects, override with `PORT`) opens `/auth/invite` with a stored session whose account is gone and presses Log out: one request, no toast, the form. Before `user/logout` joined the interceptor's `AUTH_PATHS` that was hundreds of requests and toasts a second. `npm run beehive-delete` opens the beehive delete dialog and asserts the DELETE carries the queen's fate — the API has always required it and the old plain confirm never asked, so no hive could be deleted. `npm run apiary-map` loads `/user/apiary/map` with coordinates as the strings the API really sends and asserts tiles painted without a resize; it only runs on a host the Google Maps key allows.
