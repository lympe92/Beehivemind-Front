...
| `/privacy`, `/terms` | `privacy/`, `terms/` | PageIntro → `.prose`. Terms is a **skeleton**; muted paragraphs mark values a lawyer must fill in. Both are `index, follow` and in the sitemap since 2026-09-12 (a real company behind the product is a trust signal); `/delete-account` stays `noindex`. |
