...
| `/pricing` | `pricing/` | PageIntro → PricingTiers → TextColumns (level 2) → CtaBanner. Applies its own SEO (no `seoKey`) so it can add a `FAQPage` built from the questions band — same pattern as `/help` |
